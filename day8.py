...
def get_digits(digits):
    res = {}
    res[1] = next(x for x in digits if len(x) == 2)
    digits.remove(res[1])
    res[4] = next(x for x in digits if len(x) == 4)
    digits.remove(res[4])
    res[7] = next(x for x in digits if len(x) == 3)
    digits.remove(res[7])
    res[8] = next(x for x in digits if len(x) == 7)
    digits.remove(res[8])
    res[9] = next(x for x in digits if len(x) == 6 and len(set(list(res[4])).intersection(set(list(x)))) == 4)
    digits.remove(res[9])
    res[0] = next(x for x in digits if len(x) == 6 and len(set(list(res[1])).intersection(set(list(x)))) == 2)
    digits.remove(res[0])
    res[6] = next(x for x in digits if len(x) == 6)
    digits.remove(res[6])
    res[5] = next(x for x in digits if len(x) == 5 and len(set(list(res[6])).intersection(set(list(x)))) == 5)
    digits.remove(res[5])
    res[3] = next(x for x in digits if len(set(list(res[1])).intersection(set(list(x)))) == 2)
    digits.remove(res[3])
    res[2] = digits[0]
    return {v: str(k) for k, v in res.items()}
...
def main(inp, is_real):
    ...
    for line in inp:
        ...
        digits = get_digits(first.split(' '))
        sets = {k: set(list(k)) for k, v in digits.items()}
        res = ''
        for s in second.split(' '):
            cur_set = set(list(s))
            res += next(digits[k] for k, v in sets.items() if cur_set == v)
        sum2 += int(res)
        sum += len([s for s in second.split(' ') if len(s) == 2 or len(s) == 3 or len(s) == 4 or len(s) == 7])
    print(f'part1: {sum}, part2: {sum2}')
